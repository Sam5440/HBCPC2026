...
int main(){
    ...
    vector<ll> dist(n+1);
    int timer=0;
    vector<int> st={1}, par(n+1,0), it(n+1,0); up[0][1]=1;
    while(!st.empty()){
        int u=st.back();
        if(!it[u]){
            tin[u]=++timer;
    ...
    auto dis=[&](int a,int b){int c=lca(a,b); return dist[a]+dist[b]-2*dist[c];};
    /* Independent fallback: exact scan. It is intentionally simple for validation. */
    while(q--){
        int x,y; cin>>x>>y;
        ll best=LLONG_MIN;
